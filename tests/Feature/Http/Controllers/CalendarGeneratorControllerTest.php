...
test('user can get their events list', function () {
    ...
    actingAs($user)->get(route('my-events'))->assertOk()
        ->assertInertia(fn (AssertableInertia $page) => $page
            ->component('MyEvents')
            ->has('events.data', 9)
            ->where('events.data.8.id', 9)
            ->where('events.data.8.ics', IcsEvent::find(9)->ics)
            ->where('events.data.8.summary', IcsEvent::find(9)->getSummary())
            ->where('events.data.7.id', 8)
            ->where('events.data.7.ics', IcsEvent::find(8)->ics)
            ->where('events.data.7.summary', IcsEvent::find(8)->getSummary())
            ->where('events.data.6.id', 7)
            ->where('events.data.6.ics', IcsEvent::find(7)->ics)
            ->where('events.data.6.summary', IcsEvent::find(7)->getSummary())
            ->where('events.data.5.id', 6)
            ->where('events.data.5.ics', IcsEvent::find(6)->ics)
            ->where('events.data.5.summary', IcsEvent::find(6)->getSummary())
            ...
            ->where('events.data.3.id', 4)
            ->where('events.data.3.ics', IcsEvent::find(4)->ics)
            ->where('events.data.3.summary', IcsEvent::find(4)->getSummary())
            ->where('events.data.2.id', 3)
            ->where('events.data.2.ics', IcsEvent::find(3)->ics)
            ->where('events.data.2.summary', IcsEvent::find(3)->getSummary())
            ->where('events.data.1.id', 2)
            ->where('events.data.1.ics', IcsEvent::find(2)->ics)
            ->where('events.data.1.summary', IcsEvent::find(2)->getSummary())
            ->where('events.data.0.id', 1)
            ->where('events.data.0.ics', IcsEvent::find(1)->ics)
            ->where('events.data.0.summary', IcsEvent::find(1)->getSummary())
        );
});

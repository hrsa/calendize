...
test('emails are properly verified', function () {
    ...
    post(route('user.check-email'), ['email' => 'random@email.com'])->assertOk();
    post(route('user.check-email'), ['email' => $user->email])->assertForbidden();
...
test('user can hide the password reminder', function () {
    ...
    actingAs($user)->post(route('hide-password-reminder'));
